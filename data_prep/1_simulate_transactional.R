...
source("shiny_app/R/utils.R")

# number of claims
n_claims <- 10000

# start/end dates & accident range
beg_date <- as.Date("2006-01-01")
end_date <- as.Date(end_of_month(beg_of_month(Sys.Date()) - 1))
...
fs::dir_create("data_prep/cache")
qs::qsave(trans, file = "data_prep/cache/trans.qs")
file.copy("data_prep/cache/trans.qs", "shiny_app/data/trans.qs")
